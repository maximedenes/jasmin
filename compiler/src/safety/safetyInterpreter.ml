...
module AbsInterpreter (PW : ProgWrap) : sig
  val analyze : (Format.formatter -> X86_extra.x86_extended_op Sopn.sopn -> unit) -> unit -> analyse_res
end = struct
  ...
  let split_opn pp_opn n opn es = match opn with
  ...
  (* Heuristic for the (candidate) decreasing quantity to prove while
     loop termination. *)  
  let dec_qnty_heuristic pp_opn loop_body loop_cond =
                ...
                List.find_map_opt (fun lv ->
                    ...
                    | Lvar x -> 
                      ...
                      then
                        ...
                        let heur = opn_heur pp_opn opn reg_assgn es in
  ...
  let print_ginstr pp_opn ginstr abs_vals =
    Format.eprintf "@[<v>@[<v>%a@]@;*** %d Instr: nb %d, %a %a@;@;@]%!"
      ...
      (Printer.pp_instr ~debug:false pp_opn) ginstr
  ...
  let print_if_join pp_opn cpt_instr ginstr labs rabs abs_r =
  ...
  let rec aeval_ginstr pp_opn : ('ty,minfo,'asm) ginstr -> astate -> astate =
    fun ginstr state ->
      debug (fun () ->
        print_ginstr pp_opn ginstr state.abs);
      ...
      then state
        ...
        aeval_ginstr_aux pp_opn ginstr state

  and aeval_ginstr_aux pp_opn : ('ty,minfo,'asm) ginstr -> astate -> astate =
    fun ginstr state ->
    match ginstr.i_desc with 
      | Cassgn (lv,tag,ty1, Pif (ty2, c, el, er))
        ...
        aeval_if pp_opn ginstr c [cl] [cr] state
      ...
      | Copn (lvs,_,opn,es) ->
        (* Remark: the assignments must be done in the correct order. *)
        let assgns = split_opn pp_opn (List.length lvs) opn es in
        let abs = AbsExpr.abs_assign_opn state.abs ginstr.i_info lvs assgns in

        { state with abs = abs; }

      | Cif(e,c1,c2) ->
        aeval_if pp_opn ginstr e c1 c2 state
      ...
      | Cwhile(_,c1, e, c2) ->
        ...
        let cpt = ref 0 in
        let state = aeval_gstmt pp_opn c1 state in
        ...
        let ni_e =
          try Some (dec_qnty_heuristic pp_opn (c2 @ c1) (oec state.abs))
        ...
        let eval_body state_i state =
          ...
          let state_o = aeval_gstmt pp_opn (c2 @ c1) state_i in
      ...
      | Ccall(_, lvs, f, es) ->
        ...
        let fstate = aeval_call pp_opn f f_decl callsite state_i in
  ...
  and aeval_call pp_opn : funname -> (minfo, 'asm) func -> L.i_loc -> astate -> astate =
    fun f f_decl callsite st_in ->
    let itk = ItFunIn (f,callsite) in

    match aeval_call_strategy callsite f_decl st_in with 
    | Config.Call_Direct -> aeval_body pp_opn f_decl.f_body st_in
    ...
    | Config.Call_TopByCallSite ->
      ...
      (* We abstractly evaluate f for the first time *)
        ...
        let st_out_ndisj = aeval_body pp_opn f_decl.f_body st_in_ndisj in
  ...
  and aeval_if pp_opn ginstr e c1 c2 state =
    ...
    (* Branches evaluation *)
    let lstate = aeval_gstmt pp_opn c1 { state with abs = labs; } in
    ...
    let rstate = aeval_gstmt pp_opn c2 { lstate with abs = rabs; } in

    let abs_res = AbsDom.join lstate.abs rstate.abs in
    debug (fun () ->
        print_if_join pp_opn cpt_instr ginstr lstate.abs rstate.abs abs_res);
    { rstate with abs = abs_res; }

  and aeval_body pp_opn f_body state =
    debug (fun () -> Format.eprintf "Evaluating the body ...@.@.");
    aeval_gstmt pp_opn f_body state

  and aeval_gstmt pp_opn : ('ty,'i,'asm) gstmt -> astate -> astate =
    fun gstmt state ->
    let state = List.fold_left (fun state ginstr ->
        aeval_ginstr pp_opn ginstr state)
        state gstmt in
    let () = if gstmt <> [] then
        debug (fun () ->            
  ...
  let analyze pp_opn () =
    ...
    try
      ...
      let final_st = aeval_gstmt pp_opn main_decl.f_body state in
