...
module FSPa : sig    
  val fs_pa_make : (X86_extra.x86_extended_op -> bool) -> (Format.formatter -> X86_extra.x86_extended_op Sopn.sopn -> unit) -> ('info, X86_extra.x86_extended_op) func -> (unit, X86_extra.x86_extended_op) func * Pa.pa_res
end = struct
  exception Fcall
  ...
  let fs_pa_make is_move_op pp_opn (f : ('info, 'asm) func) =
    ...
    debug (fun () ->
        Format.eprintf "SSA transform of %s:@;%a"
          f.f_name.fn_name
          (Printer.pp_func ~debug:true pp_opn) ssa_f);
