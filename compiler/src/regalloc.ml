...
let asm_equality_constraints asmop is_move_op (int_of_var: var_i -> int option) (k: int -> int -> unit)
    (k': int -> int -> unit)
...
let pp_trace pp_opn (i: int) fmt (tr: ('info, 'asm) trace) =
  let j = try Hashtbl.find tr i with Not_found -> [] in
  let pp_i fmt i =
    Format.fprintf fmt "@[<v>at %a:@;<1 2>%a@]"
      L.pp_iloc i.i_loc
      (Printer.pp_instr ~debug:true pp_opn) i
  in
...
let collect_equality_constraints_in_func
  ...
  let rec collect_instr_r ii =
    ...
    | Copn (lvs, _, op, es) -> copn_constraints asmop is_move_op int_of_var (add ii) addf lvs op es
...
let conflicts_add_one pp_opn tbl tr loc (v: var) (w: var) (c: conflicts) : conflicts =
...
module type Arch = sig
  val allocatable : Prog.var list
  val xmm_allocatable : Prog.var list
  val arguments : Prog.var list (* ou Sv.t ? *)
  val xmm_arguments : Prog.var list
  val ret : Prog.var list
  val xmm_ret : Prog.var list
  val reserved : Prog.var list
  val callee_save : Prog.var list (* ou Sv.t ? *)
  val flags : Prog.var list
...
module Regalloc (Arch : Test_arch.Arch) = struct
  ...
  let forced_registers translate_var loc nv (vars: int Hv.t) (cnf: conflicts)
    ...
    let mallocate_one x y a =
      match x with Pvar x when is_gkvar x -> allocate_one x.gv y a | _ -> ()
    in
    let id = get_instr_desc (Arch_extra.asm_opI Arch.asm_e) op in
    (* TODO: move !! *)
    let var_of_implicit v =
      match v with
      | IArflag v -> v
...
let renaming (f: ('info, 'asm) func) : (unit, 'asm) func =
  ...
  let eqc, _tr, _fr =
    collect_equality_constraints (Arch_extra.asm_opI Arch.asm_e) Arch.aparams
      "Split live range" (fun _ _ _ _ _ _ _ _ -> ()) vars nv f in
...
let global_allocation translate_var (funcs: ('info, 'asm) func list) : (unit, 'asm) func list * (funname -> Sv.t) * (var -> var) * (funname -> Sv.t) * (L.i_loc, var) Hashtbl.t * var Hf.t =
  ...
  if !Glob_options.debug then
    Format.printf "Before REGALLOC:@.%a@."
      Printer.(pp_list "@ @ " (pp_func ~debug:true Arch.pp_opn)) (List.rev funcs);
  ...
  let conflicts =
    Hf.fold (fun _fn lf conflicts ->
        collect_conflicts Arch.pp_opn vars tr lf conflicts
      )
      liveness_table
      empty_conflicts
  in
  ...
  let conflicts =
    ...
    Hf.iter (fun _fn f ->
        Liveness.iter_call_sites (fun loc _fn' _xs (s, _) ->
            ...
            | exception Not_found -> ()
            | r -> cnf := Sv.fold (conflicts_add_one Arch.pp_opn vars tr (Lmore loc) r) s !cnf
          ) f
      ) liveness_table;
    !cnf
  in
  ...
  let conflicts =
    List.fold_left (fun a f ->
        ...
        | ra ->
           List.fold_left (fun cnf x -> conflicts_add_one Arch.pp_opn vars tr Lnone ra x cnf) a f.f_args
        | exception Not_found -> a )
      conflicts funcs in
  (* Inter-procedural conflicts *)
  let conflicts =
    let add_conflicts s x = Sv.fold (conflicts_add_one Arch.pp_opn vars tr Lnone x) s in
