...
module Normal = struct  
  ...
  let pp_call env fmt lvs etyso etysi pp a =
   ...
   let rec pp_cmd asmOp pp_opn env fmt c =
    Format.fprintf fmt "@[<v>%a@]" (pp_list "@ " (pp_instr asmOp pp_opn env)) c

  and pp_instr asmOp pp_opn env fmt i = 
    match i.i_desc with 
    | Cassgn (lv, _, _ty, e) ->
    ...
    | Copn(lvs, _, op, es) ->
      ...
      let otys,itys = List.map Conv.ty_of_cty (Sopn.sopn_tout asmOp op), List.map Conv.ty_of_cty (Sopn.sopn_tin asmOp op) in
      let otys' = List.map Conv.ty_of_cty (Sopn.sopn_tout asmOp op') in
      let pp_e fmt (op,es) = 
    ...
    | Cif(e,c1,c2) ->
      Format.fprintf fmt "@[<v>if (%a) {@   %a@ } else {@   %a@ }@]"
        (pp_expr env) e (pp_cmd asmOp pp_opn env) c1 (pp_cmd asmOp pp_opn env) c2
      
    | Cwhile(_, c1, e,c2) ->
      Format.fprintf fmt "@[<v>%a@ while (%a) {@   %a@ }@]"
        (pp_cmd asmOp pp_opn env) c1 (pp_expr env) e (pp_cmd asmOp pp_opn env) (c2@c1)
  ...
  and pp_instr asmOp pp_opn env fmt i = 
    ...
    | Cwhile(_, c1, e,c2) ->
      ...
      Format.fprintf fmt "@[<v>%a%a@ while (%a) {@   %a%a@ }@]"
        (pp_cmd asmOp pp_opn env) c1 pp_leak e (pp_expr env) e 
        (pp_cmd asmOp pp_opn env) (c2@c1) pp_leak e
...
let pp_fun asmOp pp_opn env fmt f = 
  ...
  Format.fprintf fmt 
    ...
    (pp_cmd asmOp pp_opn env) f.f_body
...
let pp_prog asmOp pp_opn fmt model globs funcs = 
  ...
  Format.fprintf fmt 
    ...
    pp_leakages env 
    (pp_list "@ @ " (pp_fun asmOp pp_opn env)) funcs 
...
let extract asmOp pp_opn fmt model ((globs,funcs):('info, 'asm) prog) tokeep = 
  ...
  pp_prog asmOp pp_opn fmt model globs (List.rev funcs)
