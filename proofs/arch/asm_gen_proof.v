...
Lemma to_var_reg_neq_xreg (r : reg_t) (x : xreg_t) : to_var r <> to_var x.
Proof.
...
Proof.
  ...
  + move=> [x xii] /= hw [<-]; rewrite /arg_of_pexpr.
    ...
    5: rewrite eq_sym.
    2, 5: by apply /eqP /to_var_reg_neq_xreg.
    + exact: h3.
    1, 3: exact: h4.
    + exact: h2.
...
Lemma assemble_iP i j ls ls' lc xs :
  ...
  -> linear_sem.eval_instr p mov_eop i ls = ok ls'
  -> exists2 xs',
...
Proof.
  ...
  - case => fn lbl [<-] /=; t_xrbindP => body.
    ...
    by constructor.
  - t_xrbindP=> e _ /assertP /eqP ok_e d ok_d <- ptr v ok_v ok_ptr.
  ...
  - case => // x lbl.
    ...
    move: ok_s' ok_r_x.
    rewrite to_estate_of_estate wrepr_unsigned.
...
Lemma asm_gen_exportcall callee_saved fn m vm m' vm' :
  lsem_exportcall p mov_eop (sv_of_list to_var callee_saved) m fn vm m' vm'
...
Proof.
  ...
  1-4: rewrite /is_ok_or_narr_undef /=.
